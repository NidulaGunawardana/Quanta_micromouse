...
float ms_Kp = 2.3e-2, ms_Kd = 1.2e-4; // PD controller constants for moving straight
...
float etta_Kp = 0.6e-3, etta_Kd = 0.6e-4;
...
void turnToAngle(float angle) {
	...
	int l_target = l_start_position
			+ (angle > 0 ? target_encoder_counts : -target_encoder_counts);
	int r_target = r_start_position
			+ (angle < 0 ? target_encoder_counts : -target_encoder_counts);
	...
	while (1) {
		...
		l_pd_output =
				(fabs(l_pd_output) > 0.3) ?
						0.3 * (l_pd_output / fabs(l_pd_output)) : l_pd_output;
		l_pd_output =
				(fabs(l_pd_output) < 0.1) ?
						0.1 * (l_pd_output / fabs(l_pd_output)) : l_pd_output;

		r_pd_output =
				(fabs(r_pd_output) > 0.3) ?
						0.3 * (r_pd_output / fabs(r_pd_output)) : r_pd_output;
		r_pd_output =
				(fabs(r_pd_output) < 0.1) ?
						0.1 * (r_pd_output / fabs(r_pd_output)) : r_pd_output;
...
//		del_ir_time = (msir_curr_time - msir_prev_time) / 1000;
		...
		if (avg_distance >= 7 && wall_cap == 2) { // Wall capture when half cell distance is travelled
			updateIRValues();
...
//			HAL_Delay(2000);
		...
		} else if (avg_distance >= 6 && wall_cap == 1) {
			updateIRValues();
			L2 = disL;
			R2 = disR;
//			STOP_ROBOT;
//			HAL_Delay(2000);
			...
			wall_cap += 1;
		} else if (avg_distance >= 5 && wall_cap == 0) {
			updateIRValues();
			L3 = disL;
			R3 = disR;
...
//						ms_pd_output;
		...
		ms_ir = (fabs(ms_ir) > 0.2) ? 0.2 * (ms_ir / fabs(ms_ir)) : ms_ir;

		l_speed = ms_speed - ms_pd_output -  ms_ir;
		r_speed = ms_speed + ms_pd_output +  ms_ir;
...
//
//		// Update current robot orientation using gyro
//		float ms_current_angle = angleTurned((r_distance-l_distance)/2);  // Read current orientation
//
//		// Calculate the error: difference between the current angle and the initial angle
//		ms_current_error = ms_initial_angle - ms_current_angle;
//
...
void alignAndMaintainDistance(float targetDistance) {
	...
	while (1) {
		...
		if ((HAL_GetTick() - aamd_start_time) > 1300) {
			STOP_ROBOT
			;
			break;
		}
		...
		float derivative_align = ((error_align - previous_error_align) * 1000)
				/ (aamd_current_time - aamd_prev_time);
		...
		float derivative_distance = ((error_distance - previous_error_distance)
				* 1000) / (aamd_current_time - aamd_prev_time);
